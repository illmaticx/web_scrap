...
def get_oss(obj: contExtractor, num: int) -> list:
	...
	try:
		if "win" in ls[0]["class"]:
			win = "win"
	except IndexError:
		win = ""
	
	try:
		if "mac" in ls[1]["class"]:
			mac = "mac"
	except IndexError:
		mac = ""

	try:
		if "linux" in ls[2]["class"]:
			linux = "linux"
	except IndexError:
		linux = ""
...
def get_review_info(obj: contExtractor, num: int) -> list:
	...
	try:
		...
		# Switches used to seperate string for status and string for percent, which are in the same sentence in the container from GetRateCont
		...
		# if type(obj.GetRateCont(num).span["data-tooltip-html"]) == None, there is no review info, thus all elmements are set to be empty
		status = ""
		percent = ""
		pop = ""
...
def get_tag_info(obj: contExtractor, num: int) -> list:
	...
	# Tags obtained via steam_tag_ext.py
	...
	try:
		...
		for i in range(len(tag_keys)):
			try:
				...
				# If the key for the tag does not match any value found in steam_tags, the tag num is appended instead of what would be the name of the tag
				game_tags.append(tag_keys[i])

	except KeyError:
		# If there is no tags at all, a KeyError exception will be thrown, so game_tags will be assigned as an empty list in the event of this occuring
		game_tags = []
...
def info(data: dict) -> None:
	...
	with open(csv_file, "w") as f:
		...
		for i in range(len(data["Title"])):
			writer.writerow([data["Title"][i],
				data["Available_OS"][i],data["Month"][i],
				data["Day"][i],data["Year"][i],
				data["Current_Price"][i],data["Original_Price"][i],
