...
# Title Extraction
first_game_title = first_game.span.string

# Eligible OS Extraction (tuple) (win, mac, lin)
first_game_os_list = first_game.find("div", class_ = "col search_name ellipsis").p.find_all("span")

try:
	win = "win"	in first_game_os_list[0]["class"]
except IndexError:
	win = False
	
try:
	mac = "mac" in first_game_os_list[1]["class"]
except IndexError:
	mac = False

try:
	linux = "linux" in first_game_os_list[2]["class"]
except IndexError:
	linux = False

os_available = (win,mac,linux)	

# Release Date Extraction
first_game_release = first_game.find("div", class_ = "col search_released responsive_secondrow").string

first_game_release_year = first_game_release[-4:]
first_game_release_month = first_game_release[0:3]
first_game_release_day = first_game_release[-8:-6]

# Review Data Extraction
first_game_review_container = first_game.find("div", class_ = "col search_reviewscore responsive_secondrow")

# Review Status/Percent/Number of Reviwers (Positive/Mixed/Etc.)
review_status = ""
review_percent = ""
review_pop = ""
temp = first_game_review_container.span["data-tooltip-html"].replace("<br>", "_").replace("%", "").replace(",","").replace(" of the ", "-")
switch_one = False
switch_two = False
...
price_orig = ""
price_disc_per = ""
price_curr_cont = first_game.find("div", class_ = "col search_price responsive_secondrow")
price_disc_per_cont = first_game.find("div", class_ = "col search_discount responsive_secondrow")

# If there is no sale
if price_disc_per_cont.span == None:
	price_curr = first_game.find("div", class_ = "col search_price responsive_secondrow").text.replace(" ", "").replace("\n", "").replace("$", "")
	if len(price_curr) == 0:
		price_curr = "N/A"
	price_orig = "N/A"
	price_disc_per = "N/A"
# If there is a sale
	...
	price_curr = first_game.find("div", class_ = "col search_price discounted responsive_secondrow").text.replace("\n","").replace(" ", "").split("$")[2]
	price_orig = first_game.find("div", class_ = "col search_price discounted responsive_secondrow").text.replace("\n","").replace(" ", "").split("$")[1]
...
# tag_values = [steam_tags[i] for i in tag_keys]
# print(first_game_title, tag_values)

first_game_data = {"title":first_game_title, 
"os_list":os_available, "release_date":first_game_release, "review_status":review_status,
"review_percent":review_percent, "review_pop":review_pop,"current_price":price_curr,
